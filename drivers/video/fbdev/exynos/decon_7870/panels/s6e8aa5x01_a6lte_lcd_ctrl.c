...
try_read:
	...
	if (rx_size != size) {
		if (--retry)
			...
			dev_err(&lcd->ld->dev, "%s: fail. %02x, %d, %d\n", __func__, addr, size, rx_size);
			ret = -EPERM;
		}
	}
...
static int s6e8aa5x01_read_mtp(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_MTP, ARRAY_SIZE(buf), buf);
	if (ret < 0)
...
static int s6e8aa5x01_read_manufacture_info(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_MANUFACTURE_INFO, LDI_LEN_MANUFACTURE_INFO, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
...
static int s6e8aa5x01_read_chip_id(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_CHIP_ID, LDI_LEN_CHIP_ID, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
...
static int s6e8aa5x01_read_hbm(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_HBM, ARRAY_SIZE(buf), buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
...
#ifdef CONFIG_DISPLAY_USE_INFO
static int s6e8aa5x01_inc_dpui_u32_field(struct lcd_info *lcd, enum dpui_key key, u32 value)
{
	if (lcd->connected)
...
static int s6e8aa5x01_read_rdnumed(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_RDNUMED, LDI_LEN_RDNUMED, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
	else
		s6e8aa5x01_inc_dpui_u32_field(lcd, DPUI_KEY_PNDSIE, (buf[0] & LDI_PNDSIE_MASK));

	memcpy(&lcd->rdnumed, buf, LDI_LEN_RDNUMED);

	dev_info(&lcd->ld->dev, "%s: %x\n", __func__, lcd->rdnumed);
...
static int s6e8aa5x01_read_esderr(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_ESDERR, LDI_LEN_ESDERR, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
	else {
		s6e8aa5x01_inc_dpui_u32_field(lcd, DPUI_KEY_PNELVDE, !!(buf[0] & LDI_PNELVDE_MASK));
		s6e8aa5x01_inc_dpui_u32_field(lcd, DPUI_KEY_PNVLI1E, !!(buf[0] & LDI_PNVLI1E_MASK));
		s6e8aa5x01_inc_dpui_u32_field(lcd, DPUI_KEY_PNVLO3E, !!(buf[0] & LDI_PNVLO3E_MASK));
		s6e8aa5x01_inc_dpui_u32_field(lcd, DPUI_KEY_PNESDE, !!(buf[0] & LDI_PNESDE_MASK));
	}
...
static int s6e8aa5x01_read_rddsdr(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_RDDSDR, LDI_LEN_RDDSDR, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
	else
		s6e8aa5x01_inc_dpui_u32_field(lcd, DPUI_KEY_PNSDRE, !(buf[0] & LDI_PNSDRE_MASK));
...
static int s6e8aa5x01_read_rddpm(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_RDDPM, LDI_LEN_RDDPM, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
...
static int s6e8aa5x01_read_rddsm(struct lcd_info *lcd)
{
	...
	ret = s6e8aa5x01_read_info(lcd, LDI_REG_RDDSM, LDI_LEN_RDDSM, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
